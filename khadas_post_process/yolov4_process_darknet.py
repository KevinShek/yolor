...
def organising_pre_data(data):
    ...
    # print(show_top5(np.array(data[0][0],dtype=np.float32)))
    ...
    #GRID0 = int(sqrt(len(data[0][0]) / (LISTSIZE * SPAN)))
    #GRID1 = int(sqrt(len(data[0][1]) / (LISTSIZE * SPAN)))
    #GRID2 = int(sqrt(len(data[0][2]) / (LISTSIZE * SPAN)))
    

    input0_data = data[0][2]
    input1_data = data[0][1]
    input2_data = data[0][0]
    #input0_data = data[0][0]
    #input1_data = data[0][1]
    #input2_data = data[0][2]
...
def process(input, mask, anchors, img_size):
    ...
    box_wh = np.exp(input[..., 2:4]) # orginal
    #box_wh = sigmoid(input[..., 2:4]) # yolov7
    box_wh = box_wh * anchors
    #box_wh = (box_wh * 2) ** 2 * anchors # yolov7
...
def yolov4_post_process(data, img_size, OBJ_THRESH=0.1, NMS_THRESH=0.6, MAX_BOXES=300):
    ...
    #masks = [[6, 7, 8], [3, 4, 5], [0, 1, 2]] # yolov7
...
def draw(image, boxes, scores, classes):
    ...
    for box, score, cl in zip(boxes, scores, classes):
        list_of_image_data.append([cl, box[0], box[1], box[2], box[3], score])
        ...
        for image_type in [possible_target, current_frame]:
            ...
            cv.putText(image_type, '{0} {1:.2f}'.format(CLASSES[int(cl)], score),
                        (top, left - 6),
                        cv.FONT_HERSHEY_SIMPLEX,
                        0.6, (0, 0, 255), 2)
    ...
    return list_of_images, list_of_image_data
...
if __name__ == '__main__':
    ...
    outputs = yolov4_post_process(data, img_size=img_size, OBJ_THRESH=0.37, NMS_THRESH=0.3)
    print(outputs)
    ...
    if outputs is not None:
        for output in outputs:
          draw(img, [output[:4]], [output[4]], [output[5]])
